...
if ($_SESSION['auth_admin'] == "yes_auth")
{
    ...
    if(isset($_GET["logout"]))
    {
   	...
   	$_SESSION['urlpage'] = "<a href='index.php'>Головна</a> \ <a href='clients.php'>Клієнти</a>";
...
<head>
	...
	<title>Панель керування - Клієнти</title>
</head>
<body>
    <div id="block-body">
<?php
...
?>
    <div id="block-content">
        ...
        if($_SESSION['view_clients'] == '1')
        {
                    ...
                    if (mysql_num_rows($result) > 0)
                    {
                        ...
                        do
                        {
                            ...
                            <div class="block-clients">
                                ...
                                <p class="client-email"><strong>'.$row["surname"].' '.$row["name"].' : '.$row["email"].'</strong></p>
                                <p class="client-links"><a class="delete" rel="clients.php?id='.$row["id"].'&action=delete">Видалити</a></p>
                            
                                <ul>
                                    <li><strong>E-mail</strong> - '.$row["email"].'</li>
                                    <li><strong>ПІБ</strong> - '.$row["surname"].' '.$row["name"].' '.$row["patronymic"].'</li>
                                    <li><strong>Адреса</strong> - '.$row["address"].'</li>
                                    <li><strong>Телефон</strong> - '.$row["phone"].'</li>
                                    <li><strong>ІР</strong> - '.$row["ip"].'</li>
                                    <li><strong>Дата регістрації</strong> - '.$row["datetime"].'</li>
                                </ul>
                            </div>
                            ';
                ...
                if ($page != $total) $nextpage = '<li><span><a href="clients.php?page='. ($page + 1) .'"/>Вперед</a></span></li>';
